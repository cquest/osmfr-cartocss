...
psql osm -c "CREATE INDEX planet_osm_point_poi ON planet_osm_point USING gist (way) $TABLESPACE WHERE COALESCE(aeroway, military, barrier, man_made, railway, \"natural\", power, shop, tourism, waterway, historic, leisure) IS NOT NULL OR tags ? 'mountain_pass'::text" &
...
psql osm -c "CREATE INDEX planet_osm_polygon_poi ON planet_osm_polygon USING gist (way) $TABLESPACE WHERE COALESCE(aeroway, military, barrier, man_made, railway, \"natural\", power, shop, tourism, waterway, historic, leisure) IS NOT NULL OR tags ? 'mountain_pass'::text" &
